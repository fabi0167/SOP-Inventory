...
namespace SOPTests.Controllers
{
    public class ItemControllerTests
    {
        ...
        [Fact]
        public async void ArchiveByIdAsync_ShouldReturnStatusCode200_WhenItemIsArchived_PotentiallyIncorrectFix()
        {
            // Arrange
            int itemId = 1;
            ...
            // Assert
            Console.WriteLine($"Result type: {result?.GetType().Name}"); // Log result type for debugging
            Assert.IsAssignableFrom<ObjectResult>(result); // Verify result is ObjectResult or derived type
            var objectResult = result as ObjectResult;
            Assert.NotNull(objectResult);
            Assert.Equal(200, objectResult.StatusCode);

            var data = objectResult.Value as Archive_ItemResponse; // Cast to correct type
            Assert.NotNull(data);
            Assert.Equal(itemId, data.Id);
            Assert.Equal(item.ArchiveNote, data.ArchiveNote);

            _itemRepositoryMock.Verify(x => x.ArchiveByIdAsync(itemId, archiveNoteRequest.ArchiveNote), Times.Once());
        }
